#!/usr/bin/env npx tsx
/**
 * Betfan Market Discovery Script.
 *
 * Thin wrapper around the generic scraper-audit core. Re-scrapes one Betfan
 * match full offer, normalizes the markets, and prints RAW vs FRONTEND JSON so
 * the @betfan-audit-fixer can verify a fix (view_type / param-count check).
 *
 * Usage:
 *   npx tsx scripts/betfan-market-discovery.ts --url "<event url>" --league <slug>            # summary
 *   npx tsx scripts/betfan-market-discovery.ts --url "<event url>" --league <slug> --market "Liczba goli"  # one market detail
 *   npx tsx scripts/betfan-market-discovery.ts --url "<event url>" --league <slug> --all      # full detail for all markets
 */
...
main().catch((err) => {
  console.error("[betfan-discovery] FAILED:", err);
  process.exit(1);
});
