...
/**
 * Betters Playwright Scraper Implementation
 */
export class BettersPlaywrightScraper extends PlaywrightScraper {
  ...
  async scrapeLeague(league: string): Promise<ScraperResult> {
    ...
    try {
      ...
      console.log(`[Betters] Capturing data for league: ${league} (ID: ${leagueId})`);
  ...
  async scrapeMatchDetails(eventUrl: string): Promise<MatchDetailResult> {
    ...
    try {
      ...
      if (!event) {
        return this.createMatchDetailNotFoundResult(
          "Event not found in Betters API",
          Date.now() - startTime
        );
      }
   ...
   * EXTENDED_STAKE_TYPE_IDS, we get 20+ market types with 100+ individual markets.
   */
  async scrapeFullOffer(league: string): Promise<FullOfferScraperResult> {
    ...
    try {
      ...
      console.log(`[Betters/FullOffer] Capturing data for league: ${league}`);
   ...
   * Not used for Betters since we use API directly
   */
  async extractEventUrls(page: Page): Promise<EventUrlEntry[]> {
    // Betters uses API for data fetching, not DOM scraping
