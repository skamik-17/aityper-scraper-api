#!/usr/bin/env npx tsx
/**
 * LeBull audit data prep.
 *
 * Thin wrapper around the generic scraper-audit core. Scrapes one LeBull match
 * full offer, runs the raw markets through the LeBull normalizer, computes
 * mechanical flags, expands catalog families, and writes the intermediate JSON
 * consumed by /audit-lebull:
 *   docs/lebull-audit/.tmp/<matchId>.json  — PrepAuditOutput (gitignored)
 *
 * The stdout summary echoes the eventUrl so the orchestrator can pass it to the
 * fixer for online (re-scrape) verification.
 *
 * Usage:
 *   npx tsx scripts/lebull-prep-audit.ts --url "<lebull event url>" --league <slug> [--home "<team>"] [--away "<team>"] [--out <path>]
 */
...
function parseArgs(): Args {
  ...
  if (!url) {
    console.error(
      "Usage: --url <lebull event url> --league <slug> [--home <team>] [--away <team>] [--out <path>]",
    );
    process.exit(1);
  }
  return {
...
async function main() {
  ...
  if (!result) {
    console.error("[lebull-prep] Could not scrape/locate match for the given URL");
    process.exit(1);
  }

  const out =
    args.out ?? resolve(process.cwd(), `../docs/lebull-audit/.tmp/${result.matchId}.json`);
  mkdirSync(dirname(out), { recursive: true });
  writeFileSync(out, JSON.stringify(result.output, null, 2), "utf8");

  console.error(`[lebull-prep] Wrote ${out}`);
  console.log(
    JSON.stringify(
      {
...
main().catch((err) => {
  console.error("[lebull-prep] FAILED:", err);
  process.exit(1);
});
