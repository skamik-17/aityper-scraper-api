#!/usr/bin/env npx tsx
/**
 * Superbet audit data prep.
 *
 * Thin wrapper around the generic scraper-audit core. Scrapes one Superbet match
 * full offer, runs the raw markets through the Superbet normalizer, computes
 * mechanical flags, expands catalog families, and writes the intermediate JSON
 * consumed by /audit-superbet:
 *   docs/superbet-audit/.tmp/<matchId>.json  — PrepAuditOutput (gitignored)
 *
 * The stdout summary echoes the eventUrl so the orchestrator can pass it to the
 * fixer for online (re-scrape) verification.
 *
 * Usage:
 *   npx tsx scripts/superbet-prep-audit.ts --url "<superbet event url>" --league <slug> [--home "<team>"] [--away "<team>"] [--out <path>]
 */
...
function parseArgs(): Args {
  ...
  if (!url) {
    console.error(
      "Usage: --url <superbet event url> --league <slug> [--home <team>] [--away <team>] [--out <path>]",
    );
    process.exit(1);
  }
  return {
...
async function main() {
  ...
  if (!result) {
    console.error("[superbet-prep] Could not scrape/locate match for the given URL");
    process.exit(1);
  }

  const out =
    args.out ?? resolve(process.cwd(), `../docs/superbet-audit/.tmp/${result.matchId}.json`);
  mkdirSync(dirname(out), { recursive: true });
  writeFileSync(out, JSON.stringify(result.output, null, 2), "utf8");

  console.error(`[superbet-prep] Wrote ${out}`);
  console.log(
    JSON.stringify(
      {
...
main().catch((err) => {
  console.error("[superbet-prep] FAILED:", err);
  process.exit(1);
});
