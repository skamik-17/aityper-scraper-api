...
export const NormalizedMarketType = {
  ...
  FIRST_HALF_EUROPEAN_HANDICAP: "FIRST_HALF_EUROPEAN_HANDICAP",
    ...
    // HALF_TIME_HOME_CLEAN_SHEET retired as a catalog code (audit cluster
    // #0, findings 3/4) but kept as a type member — forbet still uses it as
    // a transient intermediate value (FORBET_TEAM_SIDED_VARIANTS) before
    // rerouting to HALF_TIME_HOME/AWAY_TO_SCORE. HALF_TIME_AWAY_CLEAN_SHEET
    // has zero remaining references and was removed outright.
  ...
  // See HALF_TIME_HOME_CLEAN_SHEET above — same retirement, same forbet
  // transient-value exception. SECOND_HALF_AWAY_CLEAN_SHEET removed outright.
  ...
  TACKLES_1X2: "TACKLES_1X2",
  FIRST_SHOT: "FIRST_SHOT",
  FIRST_SHOT_ON_TARGET: "FIRST_SHOT_ON_TARGET",
  CARDS_POINTS_OVER_UNDER: "CARDS_POINTS_OVER_UNDER",
  // HALF_TIME_CARDS_POINTS_OVER_UNDER retired — see CARDS_POINTS_OVER_UNDER's
  // comment in market-catalog.ts (audit cluster #0, finding 6).
  ...
  HALF_TIME_AWAY_TEAM_TOTAL_CARDS: "HALF_TIME_AWAY_TEAM_TOTAL_CARDS",
  ...
  // WIN_OR_BTTS / DRAW_OR_BTTS retired (audit-loop cluster #12,
  // "Result or BTTS" fragmentation): they split one real-world bet
  // ("<team> wins or BTTS" / "draw or BTTS") into up to 3 separate cards
  // per bookmaker while lebull already quoted the same bet as ONE combined
  // RESULT_OR_BTTS card (HOME_OR_BTTS_YES/NO, DRAW_OR_BTTS_YES/NO,
  // AWAY_OR_BTTS_YES/NO). etoto and superbet's per-side raw markets are now
  // routed into RESULT_OR_BTTS instead, so every bookmaker pools under one
  // key. See RESULT_OR_BTTS below.
  ...
  // Cluster #15 /audit-match: see HALF_TIME_TEAM_MULTI_GOAL_RANGE above —
  // same disjoint-partition-vs-cumulative-ladder split, mirrored for the
  // second-half-scoped variant.
  SECOND_HALF_TEAM_MULTI_GOAL_RANGE: "SECOND_HALF_TEAM_MULTI_GOAL_RANGE",
  ...
  // PLAYER_FOULS_OVER retired (audit-loop cluster #4): pooled into
  // PLAYER_FOULS's "1+".."4+" ladder — see market-catalog.ts.
  ...
  // ASSIST_SCORER_ANYTIME / PLAYER_SHOTS_ANYTIME retired (audit-loop cluster
  // #4): pooled into PLAYER_ASSISTS / PLAYER_SHOTS — see market-catalog.ts.
  ...
  // GOAL_BY_MINUTE retired (audit-loop cluster #22): inverted the
  // param/selection roles (goal-count line in paramValue, real minute
  // checkpoint buried in per-selection codes like UNDER_5MIN). superbet's
  // "Liczba goli - do X minuty" now routes to TIME_PERIOD_TOTAL_GOALS,
  // pooling with fuksiarz's identical bet family under that code's
  // "<minute>:<line>" compound param.
  // RESULT_AT_MINUTE retired (audit-loop cluster #19): superbet's "Mecz -
  // do X minuty" is the same 1-to-X-minute-window 1X2 bet every other
  // bookmaker already reports under TIME_PERIOD_RESULT — merged there so
  // it pools with those quotes instead of being a superbet-only duplicate.
  TOTAL_GOALS_OVER_PICK: "TOTAL_GOALS_OVER_PICK",
  RESULT_OR_TOTAL: "RESULT_OR_TOTAL",
  // NTH_CORNER retired (audit-loop cluster #23): superbet's "Kto pierwszy
  // wykona X rzutów rożnych" is a race-to-X-corners bet (HOME/NONE/AWAY,
  // threshold param) - the same shape betcris/lvbet already report under
  // CORNERS_RACE_TO — merged there so it pools with those quotes instead of
  // being a superbet-only duplicate.
  ...
  TEAM_WIN: "TEAM_WIN",
  // HOME_CLEAN_SHEET / AWAY_CLEAN_SHEET retired (audit cluster #0, findings
  // 3/4): "{team} won't concede" is the same proposition as "opponent to
  // score" — fortuna/etoto now route into the opposing side's
  // HOME/AWAY_TEAM_TO_SCORE with inverted YES/NO. Zero remaining references.
  ...
  // Cluster #15 /audit-match: HALF_TIME_TEAM_GOAL_RANGE's disjoint 3-way
  // partition (fuksiarz: 0-1/2-3/4+) was sharing a row with superbet/lvbet's
  // overlapping cumulative-range ladder (1-2/1-3/1-4/2-3/2-4/3-4) via the
  // grouper's "exact" param fallback — the two shapes' implied probabilities
  // don't sum comparably (partition ~1, ladder well above 1), so pooling them
  // implied a same-bet comparability that doesn't exist. Same conflation the
  // full-match GOAL_RANGE/MULTI_GOAL_RANGE split already fixed; mirrored here
  // for the half+team-scoped variant.
  HALF_TIME_TEAM_MULTI_GOAL_RANGE: "HALF_TIME_TEAM_MULTI_GOAL_RANGE",
  ...
  // AWAY_CLEAN_SHEET retired — see HOME_CLEAN_SHEET's comment above.
