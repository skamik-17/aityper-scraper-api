#!/usr/bin/env npx tsx
/**
 * Totalbet audit data prep.
 *
 * Thin wrapper around the generic scraper-audit core. Scrapes one Totalbet match
 * full offer, runs the raw markets through the Totalbet normalizer, computes
 * mechanical flags, expands catalog families, and writes the intermediate JSON
 * consumed by /audit-totalbet:
 *   docs/totalbet-audit/.tmp/<matchId>.json  — PrepAuditOutput (gitignored)
 *
 * The stdout summary echoes the eventUrl so the orchestrator can pass it to the
 * fixer for online (re-scrape) verification.
 *
 * Usage:
 *   npx tsx scripts/totalbet-prep-audit.ts --url "<totalbet event url>" --league <slug> [--home "<team>"] [--away "<team>"] [--out <path>]
 */
...
function parseArgs(): Args {
  ...
  if (!url) {
    console.error(
      "Usage: --url <totalbet event url> --league <slug> [--home <team>] [--away <team>] [--out <path>]",
    );
    process.exit(1);
  }
  return {
...
async function main() {
  ...
  if (!result) {
    console.error("[totalbet-prep] Could not scrape/locate match for the given URL");
    process.exit(1);
  }

  const out =
    args.out ?? resolve(process.cwd(), `../docs/totalbet-audit/.tmp/${result.matchId}.json`);
  mkdirSync(dirname(out), { recursive: true });
  writeFileSync(out, JSON.stringify(result.output, null, 2), "utf8");

  console.error(`[totalbet-prep] Wrote ${out}`);
  console.log(
    JSON.stringify(
      {
...
main().catch((err) => {
  console.error("[totalbet-prep] FAILED:", err);
  process.exit(1);
});
