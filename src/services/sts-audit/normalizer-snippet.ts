...
export interface SnippetOptions {
   ...
   * Default: src/services/normalization/bookmakers/sts-normalizer.ts
...
const DEFAULT_FILE_PATH = "src/services/normalization/bookmakers/sts-normalizer.ts";
...
export function extractNormalizerSnippet(
  ...
  if (matchIndices.length === 0) {
    return `// No direct references to ${marketCode} found in sts-normalizer.ts`;
  }
  ...
  const blocks: { header: string; body: string[] }[] = merged.map((r) => ({
    header: `// Lines ${r.start}-${r.end} of sts-normalizer.ts`,
