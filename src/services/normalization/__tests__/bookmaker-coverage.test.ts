...
import { normalizeScraperType } from "../core/scraper-type-mapping.js";
...
describe("Market Name Pattern Matching Coverage", () => {
  describe("All bookmaker market names should normalize correctly", () => {
    for (const { bookmaker, marketNames } of BOOKMAKER_TEST_DATA) {
      if (Object.keys(marketNames).length === 0) continue;

      describe(bookmaker, () => {
        for (const [marketName, expectedType] of Object.entries(marketNames)) {
...
describe("Market Category Assignment", () => {
  const categoryTests: Array<{ type: NormalizedMarketType; category: MarketCategory }> = [
    { type: "MATCH_WINNER", category: MarketCategory.WYNIK_MECZU },
    { type: "DOUBLE_CHANCE", category: MarketCategory.WYNIK_MECZU },
    { type: "DRAW_NO_BET", category: MarketCategory.WYNIK_MECZU },
    { type: "TOTAL_GOALS", category: MarketCategory.GOLE },
    { type: "BTTS", category: MarketCategory.GOLE },
    { type: "ODD_EVEN_GOALS", category: MarketCategory.GOLE },
    { type: "WIN_TO_NIL", category: MarketCategory.GOLE },
    { type: "CLEAN_SHEET", category: MarketCategory.GOLE },
    { type: "TEAM_TOTAL_GOALS", category: MarketCategory.GOLE },
    { type: "GOAL_RANGE", category: MarketCategory.GOLE },
    { type: "ASIAN_HANDICAP", category: MarketCategory.HANDICAP },
    { type: "EUROPEAN_HANDICAP", category: MarketCategory.HANDICAP },
    { type: "HALF_TIME_RESULT", category: MarketCategory.PIERWSZA_POLOWA },
    { type: "HALF_TIME_TOTAL_GOALS", category: MarketCategory.PIERWSZA_POLOWA },
    { type: "HALF_TIME_BTTS", category: MarketCategory.PIERWSZA_POLOWA },
    { type: "CORRECT_SCORE", category: MarketCategory.DOKLADNY_WYNIK },
    { type: "GOALSCORER_ANYTIME", category: MarketCategory.ZAWODNICY },
    { type: "CORNERS_TOTAL", category: MarketCategory.STATYSTYKI },
    { type: "CARDS_TOTAL", category: MarketCategory.STATYSTYKI },
    { type: "RESULT_AND_BTTS", category: MarketCategory.KOMBINACJE },
    { type: "HALFTIME_FULLTIME", category: MarketCategory.KOMBINACJE },
  ];

  for (const { type, category } of categoryTests) {
    it(`${type} should be in category ${category}`, () => {
      const market = {
        name: "Test",
        type,
        selections: [{ name: "Test", odds: 1.5 }],
...
describe("Selection Normalization by Bookmaker", () => {
  const selectionTests = [
    ...
    { input: "Tie", market: "Wynik meczu", expected: "DRAW" },
    { input: "Draw", market: "Wynik meczu", expected: "DRAW" },
    { input: "Remis", market: "Wynik meczu", expected: "DRAW" },
    { input: "W1", market: "Wynik meczu", expected: "HOME" },
    { input: "W2", market: "Wynik meczu", expected: "AWAY" },
...
describe("Strategy 0: Scraper Type Takes Precedence", () => {
  it("should use market.type from scraper over pattern matching", () => {
    const result = normalizer.normalize(
      {
        name: "Some Random Market Name",
        type: "1X2",
        selections: [
          { name: "1", odds: 2.0 },
          { name: "X", odds: 3.0 },
    ...
    expect(result.category).toBe(MarketCategory.WYNIK_MECZU);
  });

  it("should handle OVER_UNDER type from scraper", () => {
    const result = normalizer.normalize(
      {
        name: "Custom O/U Name",
        type: "OVER_UNDER",
        selections: [
    ...
    expect(result.category).toBe(MarketCategory.GOLE);
  });

  it("should handle Betcris P1XP2 format", () => {
    const result = normalizer.normalize(
      {
        name: "Match Result",
        type: "P1XP2",
  ...
  it("should fall back to pattern matching if type not recognized", () => {
    const result = normalizer.normalize(
      {
        name: "Wynik meczu",
...
describe("Real-World Scraper Data Simulation", () => {
  describe("Superbet API Response", () => {
    it("should handle Superbet MARKET_TYPES format", () => {
      const superbetMarkets = [
        { marketId: 547, type: "1X2", name: "Wynik meczu" },
        { marketId: 548, type: "DOUBLE_CHANCE", name: "Podwójna szansa" },
        { marketId: 539, type: "BTTS", name: "Obie drużyny strzelą" },
        { marketId: 200734, type: "OVER_UNDER", name: "Liczba goli 2.5" },
        { marketId: 549, type: "ASIAN_HANDICAP", name: "Handicap azjatycki" },
        { marketId: 556, type: "CORRECT_SCORE", name: "Dokładny wynik" },
      ];

      for (const { type, name } of superbetMarkets) {
        const result = normalizer.normalize(
          { name, type, selections: [{ name: "Test", odds: 1.5 }] },
          "superbet",
          "Team A",
          "Team B"
        );
        expect(result.normalizedType).not.toBe("OTHER");
      }
    });
  });

  describe("Betcris Swarm WebSocket Response", () => {
    it("should handle Betcris SWARM_MARKET_TYPES format", () => {
      const betcrisMarkets = [
        { swarmType: "P1XP2", expected: "MATCH_WINNER" },
        { swarmType: "1X12X2", expected: "DOUBLE_CHANCE" },
        { swarmType: "OverUnder", expected: "TOTAL_GOALS" },
        { swarmType: "BothTeamsToScore", expected: "BTTS" },
        { swarmType: "P1XP2FirstHalf", expected: "HALF_TIME_RESULT" },
        { swarmType: "HalfTimeOverUnder", expected: "HALF_TIME_TOTAL_GOALS" },
        { swarmType: "AsianHandicap", expected: "ASIAN_HANDICAP" },
        { swarmType: "EuropeanHandicap", expected: "EUROPEAN_HANDICAP" },
        { swarmType: "CorrectScore", expected: "CORRECT_SCORE" },
        { swarmType: "DrawNoBet", expected: "DRAW_NO_BET" },
        { swarmType: "OddEven", expected: "ODD_EVEN_GOALS" },
        { swarmType: "HalftimeFulltime", expected: "HALFTIME_FULLTIME" },
      ];

      for (const { swarmType, expected } of betcrisMarkets) {
        const result = normalizeScraperType(swarmType);
        expect(result).toBe(expected);
      }
    });
  });

  describe("Fuksiarz GAME_TYPES format", () => {
    it("should handle Fuksiarz numeric game types", () => {
      const fuksiarzTypes: Record<number, string> = {
        1: "1X2",
        4: "DOUBLE_CHANCE",
        8: "OVER_UNDER",
        98: "BTTS",
        2: "HALF_TIME_1X2",
        9: "HALF_TIME_OVER_UNDER",
        99: "HALF_TIME_BTTS",
        10: "CORRECT_SCORE",
        6: "DRAW_NO_BET",
        5: "EUROPEAN_HANDICAP",
      ...
      for (const [, scraperType] of Object.entries(fuksiarzTypes)) {
        const result = normalizeScraperType(scraperType);
  ...
  describe("Betfan Extended Markets", () => {
    it("should handle Betfan's additional market types", () => {
      const betfanExtended = [
        { type: "TEAM_GOALS", expected: "TEAM_TOTAL_GOALS" },
        { type: "HOME_TEAM_OVER_UNDER", expected: "TEAM_TOTAL_GOALS" },
        { type: "AWAY_TEAM_OVER_UNDER", expected: "TEAM_TOTAL_GOALS" },
        { type: "WIN_MARGIN", expected: "WINNING_MARGIN" },
        { type: "CORNERS", expected: "CORNERS_TOTAL" },
        { type: "CARDS", expected: "CARDS_TOTAL" },
        { type: "HALFTIME_FULLTIME", expected: "HALFTIME_FULLTIME" },
      ];

      for (const { type, expected } of betfanExtended) {
        const result = normalizeScraperType(type);
        expect(result).toBe(expected);
      }
    });
  });
...
describe("Polish Market Name Variations", () => {
  const polishVariations = [
    { name: "Wynik meczu", expected: "MATCH_WINNER" },
    { name: "Końcowy wynik", expected: "MATCH_WINNER" },
    { name: "Zwycięzca meczu", expected: "MATCH_WINNER" },
    { name: "Mecz", expected: "MATCH_WINNER" },
    { name: "1X2", expected: "MATCH_WINNER" },
    { name: "Podwójna szansa", expected: "DOUBLE_CHANCE" },
    { name: "Szansa podwójna", expected: "DOUBLE_CHANCE" },
    { name: "Dwójtyp", expected: "DOUBLE_CHANCE" },
    { name: "DC", expected: "DOUBLE_CHANCE" },
    { name: "Remis = zwrot", expected: "DRAW_NO_BET" },
    { name: "Zakład bez remisu", expected: "DRAW_NO_BET" },
    { name: "DNB", expected: "DRAW_NO_BET" },
    { name: "Liczba goli 2.5", expected: "TOTAL_GOALS" },
    { name: "Suma goli 2.5", expected: "TOTAL_GOALS" },
    { name: "Over/Under 2.5", expected: "TOTAL_GOALS" },
    { name: "Poniżej/powyżej 2.5 goli", expected: "TOTAL_GOALS" },
    { name: "Gole 2.5", expected: "TOTAL_GOALS" },
    { name: "O/U 2.5", expected: "TOTAL_GOALS" },
    { name: "Obie drużyny strzelą", expected: "BTTS" },
    { name: "Obie drużyny strzelą gola", expected: "BTTS" },
    { name: "Czy obie drużyny strzelą", expected: "BTTS" },
    { name: "BTTS", expected: "BTTS" },
    { name: "GG/NG", expected: "BTTS" },
    { name: "Wynik 1. połowy", expected: "HALF_TIME_RESULT" },
    { name: "Pierwsza połowa 1X2", expected: "HALF_TIME_RESULT" },
    { name: "HT wynik", expected: "HALF_TIME_RESULT" },
    { name: "Połowa/Mecz", expected: "HALFTIME_FULLTIME" },
    { name: "HT/FT", expected: "HALFTIME_FULLTIME" },
    { name: "Wynik 1. i 2. połowy", expected: "HALFTIME_FULLTIME" },
    { name: "Dokładny wynik", expected: "CORRECT_SCORE" },
    { name: "Wynik dokładny", expected: "CORRECT_SCORE" },
    { name: "CS", expected: "CORRECT_SCORE" },
    { name: "Handicap azjatycki", expected: "ASIAN_HANDICAP" },
    { name: "AH -1.5", expected: "ASIAN_HANDICAP" },
    { name: "Handicap europejski", expected: "EUROPEAN_HANDICAP" },
    { name: "EH +1", expected: "EUROPEAN_HANDICAP" },
    { name: "Handicap +1", expected: "EUROPEAN_HANDICAP" },
  ];
