/**
 * Test script for Fortuna only
 */
import { fortunaScraper } from "../scrapers/bookmakers/fortuna.js";

async function testFortuna() {
  console.log("--- Testing Fortuna Premier League Listing ---");
  try {
    const res = await fortunaScraper.scrapeLeague("premier-league");
    if (res.status === "success" && res.data) {
      ...
      if (res.data[0].eventUrl) {
        console.log("\n--- Testing Fortuna Match Details ---");
        const details = await fortunaScraper.scrapeMatchDetails(res.data[0].eventUrl);
